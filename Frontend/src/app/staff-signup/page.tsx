  ...
  useEffect(() => {
    ...
    const validate = async () => {
      try {
        setLoading(true);
        setError(null);
        const res = await fetch(`${API_BASE}/api/staff-invitations/validate?token=${encodeURIComponent(token)}`);
        if (!res.ok) {
        ...
        {!loading && invitation && !successMessage && (
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={submitting}
              className="w-full mt-2 inline-flex justify-center items-center px-4 py-2.5 rounded-lg bg-[#0066cc] text-white text-sm font-medium hover:bg-[#0052a3] disabled:opacity-60"
            >
        ...
        {successMessage && (
          <div className="space-y-4 text-center mt-4">
            ...
            <button
              type="button"
              onClick={goToLogin}
              className="inline-flex justify-center px-4 py-2 rounded-lg border border-gray-300 text-sm font-medium text-gray-700 hover:bg-gray-50"
            >
              Go to login
            </button>
          </div>
